...
func (aisuo *AppInvitationSettingUpdateOne) Save(ctx context.Context) (*AppInvitationSetting, error) {
	...
	if len(aisuo.hooks) == 0 {
		...
		v, err := mut.Mutate(ctx, aisuo.mutation)
		if err != nil {
			return nil, err
		}
		nv, ok := v.(*AppInvitationSetting)
		if !ok {
			return nil, fmt.Errorf("unexpected node type %T returned from AppInvitationSettingMutation", v)
		}
		node = nv
	}
	return node, err
