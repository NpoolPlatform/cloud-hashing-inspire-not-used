    ...
    stage('Unit Tests') {
      ...
      steps {
        ...
        sh (returnStdout: false, script: '''
          ...
          servicename="cloud-hashing-stimulate"

          PASSWORD=`kubectl get secret --namespace "kube-system" mysql-password-secret -o jsonpath="{.data.rootpassword}" | base64 --decode`
          kubectl -n kube-system exec mysql-0 -- mysql -h 127.0.0.1 -uroot -p$PASSWORD -P3306 -e "create database if not exists stimulates;"
          ...
          for vhost in `cat cmd/*/*.viper.yaml | grep hostname | awk '{print $2}' | sed 's/"//g' | sed 's/\\./-/g'`; do
            ...
            ./apollo-item-config.sh $APP_ID $TARGET_ENV $vhost database_name stimulates
            cd -
          done
    ...
    stage('Generate docker image') {
      ...
      steps {
        sh(returnStdout: true, script: '''
          images=`docker images | grep entropypool | grep cloud-hashing-stimulate | awk '{ print $3 }'`
